...
local function validate_utf8(input, toml_sub)
	...
	while i <= len do
		...
		if byte <= 127 then
			if toml_sub then
				if byte < 9 then
					_nsmerror("TOML only allows some control characters, but they must be escaped in double quoted strings", input,
						i, "string")
...
local function handle_backslash_escape(sm)
	...
	if not sm.match then
		...
		"^(U)([0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F])", sm.i + 1)
	end
	if sm.match then
...
local function remove_underscores_number(sm, number, anchor)
	if number:find("_") then
		...
		if number:find("^_") or number:find("_$") then _error(sm,
				"Underscores are not allowed at beginning or end of a number. Found " .. anchor .. " : " .. number, anchor) end
...
local function validate_integer(sm, value)
	...
	if sm.match then
		if sm.match:find("^[-+]?0[%d_]") then _error(sm, "Integers can't start with a leading 0. Found integer: " .. sm
			.match, "integer") end
...
local function validate_datetime(sm, value)
	...
	if sm.match then
		if _tointeger(hour) > 23 then _error(sm, "Hours must be less than 24. Found hour: " .. hour .. "in: " .. sm.match,
				"local-time") end
		if _tointeger(min) > 59 then _error(sm, "Minutes must be less than 60. Found minute: " .. min .. "in: " .. sm.match,
				"local-time") end
		if _tointeger(sec) > 60 then _error(sm, "Seconds must be less than 61. Found second: " .. sec .. "in: " .. sm.match,
				"local-time") end
		if sm.ext ~= "" then
	...
	if sm.match then
		...
		if day == 0 or day > max_days_in_month[month] then _error(sm,
				...
				day .. " days in month " .. month .. ", which only has " .. max_days_in_month[month] .. " days in: " .. sm.match,
				"local-date") end
		if month == 2 then
			...
			if leap_year == false then
				if day > 28 then _error(sm,
						...
						day .. " days in month 02, which only has 28 days if it's not a leap year in: " .. sm.match, "local-date") end
			end
		end
	...
	if sm.match then
		...
		if month == 2 then
			...
			if leap_year == false then
				if day > 28 then _error(sm,
						"Too many days in month. Found " ..
						day .. " days in month 02, which only has 28 day if it's not a leap year in: " .. sm.match, "local-datetime") end
			end
		end
